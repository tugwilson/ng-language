...
public class Divide extends BaseBinaryOperation implements FloatBinaryArithmeticOperation {
  ...
  public Object doApply(final ThreadContext tc, final float lhs, final BigInteger rhs) {
    return tc.wrap(BigDecimal.valueOf(lhs).divide(new BigDecimal(rhs.toString())).floatValue());
  }
  ...
  public float doFloatApply(final ThreadContext tc, final float lhs, final BigInteger rhs) {
    return BigDecimal.valueOf(lhs).divide(new BigDecimal(rhs.toString())).floatValue();
  }
