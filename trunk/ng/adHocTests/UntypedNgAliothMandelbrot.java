...
//      final double Cr = (x*fac - 1.5);
        Object Cr = tc.subtract().apply(tc.multiply().apply(x, fac), 1.5);
//      final double Ci = (y*fac - 1.0);
        Object Ci = tc.subtract().apply(tc.multiply().apply(y, fac), 1.0);
...
//        } while (!(ZiN + ZrN > limitSquared) && --i > 0);
        } while (!(tc.greaterThan().applyBoolean(tc.add().apply(ZiN, ZrN), limitSquared)) &&
            tc.greaterThan().applyBoolean((i = tc.subtract().apply(i, 1)), 0));
