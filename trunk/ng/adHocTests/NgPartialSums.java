...
public class NgPartialSums {
  ...
  public static void main(final String[] args) throws Throwable {
    ...
    while (tc.lessThanOrEquals().applyBoolean(k, n)) {
      ...
      try {
        a1 = tc.add().doubleApply(a1, tc.power().doubleApply(NgPartialSums.twothirds, tc.subtract().doubleApply(k, 1.0)));
      } catch (final NotPerformed e8) {
        a1 = tc.convert().asDouble(tc.add().apply(a1, tc.power().apply(NgPartialSums.twothirds, tc.subtract().apply(k, 1.0))));
      }
